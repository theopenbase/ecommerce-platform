...
import (
	...
	"github.com/ecommerce/order-service/internal/handler"
...
func main() {
	...
	repo := repository.NewOrderRepository(db, rdb)
	goodsCli := goodsclient.NewGoodsClient(cfg.GoodsHost)
	svc := service.NewOrderService(repo, goodsCli)
	orderHandler := handler.NewOrderHandler(svc, rdb)
