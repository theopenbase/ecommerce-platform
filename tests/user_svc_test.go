...
func TestRegister_Success_CreatesUserAndInitializesRecords(t *testing.T) {
	...
	mockRepo.On("Create", context.Background(), mock.AnythingOfType("*model.User")).
		Return(nil)
	mockRepo.On("CreateMember", context.Background(), mock.AnythingOfType("*model.Member")).
		Return(nil)
	mockRepo.On("CreatePointsAccount", context.Background(), mock.AnythingOfType("*model.PointsAccount")).
		Return(nil)
	mockRepo.On("DelSMSCode", context.Background(), "13800138000").
		Return(nil)

	// Register flow: verify code → check not exists → create user → init member & points
	code, _ := mockRepo.GetSMSCode(context.Background(), "13800138000")
	...
	err := mockRepo.Create(context.Background(), &model.User{Mobile: "13800138000", Nickname: "TestUser"})
	assert.NoError(t, err)
...
func TestCreateAddress_SetAsDefault_ClearsOtherDefaults(t *testing.T) {
	...
	mockRepo.On("CountAddress", context.Background(), uint64(1)).Return(0, nil)
	mockRepo.On("ClearDefaultAddress", context.Background(), uint64(1)).Return(nil)
	mockRepo.On("CreateAddress", context.Background(), mock.AnythingOfType("*model.ReceiverAddress")).Return(nil)

	// When IsDefault=1, clear existing defaults first
	mockRepo.ClearDefaultAddress(context.Background(), 1)
...
func TestSetDefaultAddress_ClearsOldThenSetsNew(t *testing.T) {
	...
	// Clear all defaults → set new default
...
func TestUserErrorConstants(t *testing.T) {
	...
	assert.Equal(t, "invalid or expired verification code", ErrInvalidCode.Error())
	assert.Equal(t, "incorrect password", ErrIncorrectPassword.Error())
	assert.Equal(t, "address not found", ErrAddressNotFound.Error())
	assert.Equal(t, "address limit reached (max 20)", ErrAddressLimitReached.Error())
}
