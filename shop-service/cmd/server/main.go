...
import (
	...
	"github.com/ecommerce/shop-service/internal/config"
	"github.com/ecommerce/shop-service/internal/handler"
...
func main() {
	...
	repo := repository.NewShopRepository(db, rdb)
	svc := service.NewShopService(repo)
	shopHandler := handler.NewShopHandler(svc)
	...
	shopHandler.RegisterRoutes(router)

	srv := &http.Server{Addr: fmt.Sprintf(":%s", cfg.Server.Port), Handler: router}
	go func() {
		log.Printf("shop-service starting on :%s\n", cfg.Server.Port)
